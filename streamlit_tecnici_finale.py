...
def color_semaforo(val, reverse=False):
    try:
        if pd.isna(val):
            return ''
        if reverse:
            return 'background-color: #ff9999' if val > 0.05 else 'background-color: #ccffcc'
        else:
...
st.dataframe(
    daily.style
        ...
        .applymap(lambda v: color_semaforo(v, reverse=True), subset=["% Rework", "% PostDelivery"])
        .applymap(lambda v: color_semaforo(v, reverse=False), subset=["% Produttivi"]),
